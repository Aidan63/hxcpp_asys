...
class TestFileSystemSetTimes extends DirectoryTests {
    function test_null_path(async:Async) {
        FileSystem.setTimes(null, 0, 0, (_, error) -> {
    ...
    function test_negative_access_time(async:Async) {
        FileSystem.setTimes(dummyFileName, -1, 0, (_, error) -> {
            if (Assert.isOfType(error, ArgumentException)) {
    ...
    function test_negative_modification_time(async:Async) {
        FileSystem.setTimes(dummyFileName, 0, -1, (_, error) -> {
            if (Assert.isOfType(error, ArgumentException)) {
    ...
    function test_modifying_directory_time(async:Async) {
        FileSystem.setTimes(directoryName, 100, 200, (_, error) -> {
    ...
    function test_modifying_file_time(async:Async) {
        ...
        FileSystem.setTimes(dummyFileName, accessTime, modificationTime, (_, error) -> {
            if (Assert.isNull(error)) {
                FileSystem.info(dummyFileName, (stat, error) -> {
                    if (Assert.isNull(error)) {
